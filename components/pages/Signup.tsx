...
export const Signup: React.FC = () => {
  ...
  const handleEmailSignup = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      // For now, just navigate to orders on successful signup
  ...
  const handleGoogleSignup = () => {
    // Dummy Google signup - navigate to orders
    try { localStorage.setItem('is_authenticated', 'true'); } catch {}
    navigate('/menu');
  };
  ...
  return (
    <div className="max-w-md mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="text-center space-y-4">
        <h2 className="text-4xl font-black text-slate-900 tracking-tight">Join the Fun</h2>
        <p className="text-slate-500 text-lg font-medium">Create your burger account</p>
      </div>
